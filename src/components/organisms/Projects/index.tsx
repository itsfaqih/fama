  ...
  return (
    <section id="projects" className={className}>
      ...
      <div className="relative grid grid-cols-1 gap-8 mt-6 sm:grid-cols-2 lg:grid-cols-3">
        <Decorations.Bullets className="absolute w-32 h-32 text-indigo-300 md:w-40 md:h-40 -top-8 -left-12 md:-top-9 md:-left-16 -z-10" />
        <Decorations.Bullets className="absolute w-32 h-32 text-indigo-300 md:w-40 md:h-40 -bottom-8 -right-12 md:-bottom-9 md:-right-16 -z-10" />
        {items.map((item, index) => (
          <ProjectCard
            key={index}
            data={item}
            className="h-full transition-all transform hover:shadow-lg hover:scale-110"
          />
        ))}
      </div>
